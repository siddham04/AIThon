...
| B-C3 | Critical | `app/services/showcase_project.py:212-225`, `bootstrap.py:63-64` | `asyncio.run()` inside async lifespan — caught exception, showcase project boots without PRD on Render | **FIXED post-audit** (added `_run_coro_blocking` helper that uses a worker thread when already inside a running loop; verified by local boot — log now shows `Pre-baked showcase project proj_demo_seed01` instead of the previous `Showcase PRD pre-bake skipped` + `RuntimeError: asyncio.run() cannot be called from a running event loop`) |
...
| B-H8 | High | `app/agents/ambiguity.py:64-79`, `test_architect.py:65-79` | Two agents bypass `LLMService` and call `get_ai_service()` directly — when developer has Azure keys in `.env`, `ai.enabled=True` and these agents hit the live LLM regardless of the `use_ai=False` parameter on `run_demo`. Surfaced as a **32-minute** golden-pipeline test run failing with "0% of tasks cite a real clause" (LLM returned `AC1/AC2/AC3` acceptance-criteria IDs that don't exist in `source_clauses`). | **FIXED post-audit** (added `helix_use_ai: bool` field to `Settings` defaulting to True; `AIService.enabled = is_configured AND helix_use_ai`; conftest sets `HELIX_USE_AI=false` AND busts the `get_settings.cache_clear()`. Verified: same suite that took 32 min and failed now passes 28/28 in 1.93 s with simulated `.env` Azure keys exported.) |
...
| **Maintainability** | **8 / 10** | Clean module boundaries (orchestrator vs agents vs services vs routes), Pydantic models everywhere, type hints throughout. Two agents bypass `LLMService` (B-H8) — still a small refactor target for symmetry, but the `helix_use_ai` kill-switch added post-audit means the bypass no longer matters for correctness. Test coverage on the core pipeline is excellent (28 invariants); breadth coverage is lower. |
