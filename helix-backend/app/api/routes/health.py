...
async def health() -> dict:
    """Public liveness probe + demo runtime hints (no secrets, no auth state).

    Frontend uses ``demo_fast`` and ``showcase_project_id`` to render the demo
    UI before the user has a session, so we surface them here.
    """
